# -*- coding: utf-8 -*-
"""
Created on Mon Jun 25 13:22:22 2018
...
class KafkaPushListener(threading.Thread):
    def __init__(self):
		#localhost:9092 = Default Zookeeper Producer Host and Port Adresses
        self.client = pykafka.KafkaClient('localhost:9092')
        #Get Producer that has topic name is Twitter
        #twitter=bytes("twitter")
        self.producer = self.client.topics["logs"].get_producer()
...
def main():
    ...
    while (flag):
        ...
        if log_format == "CLF":
            log_text = '%s - - [%s %s] "%s %s HTTP/1.0" %s %s\n' % (ip,dt,tz,vrb,uri,resp,byt)
        elif log_format == "ELF":
            log_text = '%s - - [%s %s] "%s %s HTTP/1.0" %s %s "%s" "%s"\n' % (ip,dt,tz,vrb,uri,resp,byt,referer,useragent)
        l.on_data(log_text)
        #conn.send(log_text + '\n')
